...
Control Center — **владелец** списка источников. Travel Lead Radar в будущем
станет только исполнителем: возьмёт активные источники, соберёт публикации и
сложит их в свою `leads.db`.

Важно: `enabled` означает **разрешение** использовать источник, а не факт
мониторинга. Реальный сбор требует ещё и сборщика для платформы, а кода сбора
в этом репозитории нет. **Radar пока не читает этот реестр** — он работает со
своим `app/sources/sources.json`, поэтому правка `config/sources.json` сейчас
не меняет реальный сбор.

Сейчас в реестре 24 записи: 2 собственных веб-ресурса, 16 источников Radar
(перенесены заранее, чтобы подготовить миграцию без потери мониторинга) и
6 новых Telegram-каналов — они пока **выключены** как кандидаты.
